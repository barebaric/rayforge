"""The knife-head selection VarSet variable."""
...
class KnifeHeadVar(ChoiceVar):
    """
    A special ChoiceVar that dynamically populates its choices with
    the names of the knife heads (matching *capability*) from the
    currently active machine.

    It also handles the mapping between human-readable names (for the
    UI) and the UIDs (for data storage).
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize a new KnifeHeadVar instance.

        Args:
            capability: The MachineCapability the head must provide.
            ...
            default: The default value (a knife head UID).
        ...
        if active_machine and active_machine.heads:
            knife_heads = [
                ...
                if h.machine_capability is capability
            ]
            self.name_to_uid_map = {h.name: h.uid for h in knife_heads}
            self.uid_to_name_map = {h.uid: h.name for h in knife_heads}
