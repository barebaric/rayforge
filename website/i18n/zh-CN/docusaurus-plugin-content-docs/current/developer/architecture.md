...
- **核心/服务层**：提供基础服务和状态。`Core Models` 表示文档，`Tasker` 管理后台作业，`Machine`
  处理设备通信，`Camera` 管理视口。
...
文档模型是基于 **组合模式** 的对象层次树。此结构表示用户的整个项目，从根 `Doc` 对象到各个
`WorkPiece`。它被设计为响应式和可序列化的。
...
流水线将文档模型转换为机器可执行的 G-code。它在后台异步运行，使用共享内存 `Artifact`
系统在进程之间进行高性能数据传输。流水线由多个阶段组成：**修改器 → 生产者 → 转换器 → 编码器**。
