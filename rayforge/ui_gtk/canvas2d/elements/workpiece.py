...
class WorkPieceElement(CanvasElement):
    ...
    def trigger_view_update(self):
        """
        Invalidates resolution-dependent caches and triggers a re-render.
        This is called on view changes like zooming.
        """
        logger.debug(f"View update for workpiece '{self.data.name}'")
        # Invalidate the base image buffer.
        self._surface = None
        # Request new view artifacts at the current resolution.
    ...
    def _request_view_artifact_updates(
        ...
        for step in steps_to_process:
            ...
            self.pipeline.request_view_render(
                ...
                self.data.uid,
                context,
                source_handle=cast(WorkPieceArtifactHandle, source_handle),
            )
