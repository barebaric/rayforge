---
description:
  "首次配置您的激光切割机或雕刻机。使用配置向导创建您的机器，然后连接并准备好在 Rayforge
  中进行切割。"
---
...
从应用程序菜单启动 Rayforge，或在终端中运行
`rayforge`。在首次启动时——当尚未配置任何真实机器时——配置向导会自动打开，以便您可以设置机器而无需在菜单中查找。（您始终可以稍后从
**设置 → 机器 → 添加机器** 打开它。）
...
在开始发现之前，向导会检查 Rayforge 是否能实际打开您的串口和相机。如果设备存在但缺少访问权限，会首先显示一个**权限页面**，说明如何在您的平台上修复：

- **Snap 安装**：授予 `serial-port`
  接口（以及相机接口，如需要）——确切的命令会显示并带有一键复制按钮。
...
- 发现的设备在找到可靠匹配时会**匹配到内置配置文件**，因此您通常只需确认检测到的设置，而无需手动输入。
- GRBL 从固件的编译标志自动选择正确的 G 代码方言，OctoPrint/Smoothieware 通过网络探测。
...
选择一个内置设备配置文件以预填控制器、工作区域和机头设置，或点击 **Device Not Listed**
手动配置所有内容。您也可以 **Import from File…**
导入之前导出的配置文件或带有相机校准和激光设置的 LightBurn 设备配置文件 (.lbdev)。

![向导 — 选择起点](/screenshots/config-wizard-profile.webp)

### 选择控制器

选择与您机器控制器板匹配的固件或协议系列（GRBL、Marlin、Smoothie、Ruida、OctoPrint、…）。如果您只想将 G-code 导出到文件而不驱动物理机器，请选择
**None — G-code export only**。当您从内置配置文件或导入开始时，此步骤会被跳过。
...
当您的控制器支持时，向导会提供连接设备并自动读取其配置的选项——工作区域、速度、加速度和固件功能。这既可通过 USB 串口**也可通过网络**实现（OctoPrint 和 ESP3D 的 mDNS 发现）。点击
**Probe Now** 自动检测这些值，或使用 **Next** 在后续步骤中手动输入。
...
如果您的机器是已知的商业型号，AI 可以从制造商的文档中预填其规格。输入供应商和型号，然后点击 **Look
Up Specs**。建议的值以开关行形式出现并默认接受——关闭您不想应用的任何内容。
...
- **轴方向** — 如果坐标出现负值，则反转某个轴
- **Z 轴**
  — 机器是否有 Z 轴（对焦电机、可移动床台）；如果不存在，则不生成 Z 移动，3D 画布将内容分层放置在雕刻平面上
- **面板方向** — 旋转平面工作区在屏幕上的呈现方式（原生、向左旋转、向右旋转）；旋转层需要原生
...
声明安装在龙门架上的部件——Laser Head 或 Spindle
Head——并设置其参数。对于激光：最大功率（S 值）、光斑大小、PWM 频率和焦距。对于主轴：最大和最小 RPM。
...
可选地启用您想用于预览和对齐的任何相机。当您启用相机并继续时，[相机向导](../machine/camera.md#第-2-步相机向导)
将打开，引导您完成图像设置、镜头校准和图像对齐。您可以跳过此步骤，稍后从机器的相机设置中设置相机。
...
点击 **Create Machine**
完成。机器设置对话框将为您的机器打开，您可以在其中调整向导预填的任何设置。有关详细信息，请参阅[机器设置](../machine/general.md)页面。
...
:::success 已连接！如果您的机器显示"已连接"状态，您就可以开始使用 Rayforge 了！:::
...
- **Linux（串口）**：将用户添加到 `dialout`
  组。对于**基于 Debian 的发行版上的 Snap 和非 Snap 安装**，都需要此操作以避免 AppArmor
  DENIED 消息：
