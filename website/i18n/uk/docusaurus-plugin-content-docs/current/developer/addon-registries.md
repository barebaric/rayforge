...
Реєстри — це спосіб, яким Rayforge керує розширюваністю. Кожен реєстр містить колекцію споріднених
компонентів — кроків, продюсерів, дій тощо. Коли ваш аддон реєструє щось, це стає доступним у всьому
додатку.

## Як працюють реєстри

Всі реєстри слідують схожому шаблону. Вони надають метод `register()` для додавання елементів та
різні методи пошуку для їх отримання. Більшість реєстрів також відстежують, який аддон зареєстрував
кожен елемент, щоб вони могли очистити все при вивантаженні аддону.
...
Параметр `addon_name` опціональний, але рекомендований. Він гарантує, що ваші компоненти будуть
належним чином видалені, якщо користувач відключить ваш аддон.

## Реєстр кроків

Реєстр кроків (`StepRegistry`) керує типами кроків, які з'являються в панелі операцій. Кожен крок
представляє тип операції, яку користувачі можуть додати до свого завдання.
...
Ім'я класу кроку використовується як ключ реєстру. Ваш клас кроку має успадковуватися від `Step` та
визначати атрибути на кшталт `TYPELABEL`, `HIDDEN`, а також реалізовувати метод класу `create()`.
...
Реєстр продюсерів (`ProducerRegistry`) керує продюсерами операцій. Продюсери генерують траєкторії
операцій для кроку — по суті, вони конвертують вашу робочу деталь у інструкції для машини.
...
Реєстр трансформерів (`TransformerRegistry`) керує трансформерами операцій. Трансформери виконують
постобробку операцій після того, як продюсери їх згенерували — подумайте про такі завдання, як
оптимізація шляху, згладжування або додавання утримуючих табів.
...
Реєстр дій (`ActionRegistry`) керує діями вікна. Дії — це спосіб додавання пунктів меню, кнопок
панелі інструментів та клавіатурних скорочень. Це один із найбільш функціонально насичених реєстрів.
...
Реєстр команд (`CommandRegistry`) керує командами редактора. Команди розширюють функціональність
...
Реєстр типів ресурсів (`AssetTypeRegistry`) керує типами ресурсів, які можуть зберігатися в
документах. Це забезпечує динамічну десеріалізацію — коли Rayforge завантажує документ, що містить
ваш власний ресурс, він знає, як його реконструювати.
...
`type_name` — це рядок, що використовується в серіалізованих документах для ідентифікації вашого
типу ресурсу.
...
Реєстр стратегій компонування (`LayoutStrategyRegistry`) керує стратегіями компонування для
розміщення контенту в редакторі документів.
...
Зауважте, що UI-метадані, такі як мітки та скорочення, мають реєструватися через реєстр дій, а не
тут.
...
Реєстр імпортерів (`ImporterRegistry`) керує імпортерами файлів. Імпортери обробляють завантаження
зовнішніх файлів у Rayforge.
...
Ваш клас імпортера має визначати атрибути класу `extensions` та `mime_types`, щоб реєстр знав, які
файли він обробляє.
...
Реєстр експортерів (`ExporterRegistry`) керує експортерами файлів. Експортери обробляють збереження
документів або операцій Rayforge у зовнішні формати.
...
Реєстр рендерерів (`RendererRegistry`) керує рендерерами ресурсів. Рендерери відображають ресурси в
UI.
...
Зауважте, що ви реєструєте екземпляр рендерера, а не клас. Ім'я класу рендерера використовується як
ключ реєстру.
...
Менеджер бібліотек (`LibraryManager`) керує бібліотеками матеріалів. Хоча технічно це не реєстр, він
слідує схожим шаблонам для реєстрації бібліотек, наданих аддонами.
...
Зареєстровані бібліотеки за замовчуванням доступні лише для читання. Користувачі можуть переглядати
та використовувати матеріали, але не можуть змінювати їх через UI.
