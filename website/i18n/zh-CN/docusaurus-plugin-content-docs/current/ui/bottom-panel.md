...
1. **可停靠标签**：使用左侧的图标栏在控制台、G 代码查看器和资产之间切换
2. **点动控制**：手动移动和定位（始终可见）
3. **机器状态**：实时位置和连接状态
4. **工作坐标系（WCS）**：快速 WCS 选择（始终可见）

面板的每个区域左侧都有一个图标标签栏，让您在 **控制台**、**G 代码查看器** 和 **资产**
浏览器之间切换。右侧的点动控制和 WCS 控制无论哪个标签处于活动状态都保持可见。标签可以在其标签栏内通过拖动重新排序，您还可以将标签拖到其他面板区域或分隔线上，以将布局重新排列为多列。空列会自动移除。

![底部面板](/screenshots/bottom-panel-console.webp)
...
除点动外，还可以将激光头移动到精确位置。所有这些移动都使用上文所述的**点动速度**设置。

### 移动到位置弹出面板

点击当前坐标行中的指南针图标，打开"移动到位置"弹出面板。面板会预填当前位置，坐标在当前工作坐标系中输入，与位置读数一致：

| 控件            | 功能                                         |
| --------------- | -------------------------------------------- |
| X / Y / Z       | 目标坐标（仅带 Z 轴的机器提供 Z）            |
| 移动            | 将激光头移动到输入的坐标                     |
| 左下角          | 移动到所选内容（无选择时为工作区域）的左下角 |
| 中心            | 移动到所选内容（无选择时为工作区域）的中心   |
| 右上角          | 移动到所选内容（无选择时为工作区域）的右上角 |
| 移动到 WCS 原点 | 移动到当前 WCS 的 X0 Y0                      |

机器离线时，弹出面板及其快捷方式均被禁用。

### 点击画布移动激光头

点击当前坐标行中的移动图标（或按
<kbd>Ctrl+M</kbd>）启动该模式。下一次在画布空白处左键点击会将激光头移动到该点，随后模式自动退出。右键点击会取消模式而不移动激光头。启动此模式会停用"点击归零"，反之亦然。

### 移动激光头到此处

在画布空白处右键点击，会提供"移动激光头到此处"选项，将激光头直接移动到所点击的位置。
...
当控制台或 G 代码查看器标签处于活动状态时，您还可以按 <kbd>Ctrl+F</kbd> 在内容中搜索。
...
下拉菜单列出所有已配置的激光头。当您的机器有多个激光头时，选择您想要控制的那个。副标题显示所选激光头的工具编号和最大功率。
