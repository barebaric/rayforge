...
Рівень зрілості вашого аддону. Використовуйте `experimental` для аддонів, які ще не завершені та можуть мати невирішені проблеми. Експериментальні аддони відображаються зі спеціальним значком у менеджері аддонів. Значення за замовчуванням — `stable`; для стабільних аддонів поле можна не вказувати.
