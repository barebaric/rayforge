---
description:
  "Crie cópias com os modos Grade, Rotação Pontual e Circular. Cada modo oferece visualização ao
  vivo e posicionamento interativo."
---

# Arrays

A funcionalidade de Array permite criar múltiplas cópias de peças de trabalho selecionadas usando
três modos de layout diferentes. Cada modo abre um diálogo não modal, permitindo que você continue
interagindo com a tela enquanto ajusta os parâmetros — a visualização é atualizada em tempo real.
...
O modo Grade organiza as cópias em uma matriz retangular de linhas e colunas, com espaçamento
horizontal e vertical configurável.
...
| **Modo de espaçamento**    | Escolha entre _Espaço_ (espaço entre cópias) ou _Passo_ (distância de borda a borda de cada cópia) |
...
O modo Rotação Pontual cria cópias girando-as no próprio centro da seleção. Isso é útil para criar
padrões circulares onde cada cópia permanece em sua localização original mas é girada por uma fração
do ângulo total.
...
O modo Circular posiciona cópias ao longo de um arco circular ao redor de um ponto central. Um
marcador em cruz na tela mostra o centro, e você pode arrastá-lo para uma nova posição enquanto o
diálogo está aberto.
