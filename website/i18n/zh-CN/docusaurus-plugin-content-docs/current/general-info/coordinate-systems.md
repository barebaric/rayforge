...
MACHINE 空间是相对于机器原点位置的绝对坐标系。原点 (0,0) 由机器的归零配置决定。
...
例如，如果您的机器是 400×300mm，但四周有 10mm 的边距，WORKAREA 将是 380×280mm，从 MACHINE 空间的位置 (10, 10) 开始。
...
每个图层可以独立地分配自己的工作坐标系。当图层被分配了 WCS 时，该图层的操作将相对于图层的 WCS 而非全局 WCS 进行定位。这意味着您可以：

- 在一个作业中在不同位置运行不同图层
- 设置多个夹具并分配给特定图层
- 在同一项目中组合平面和旋转工作，每个都有自己的坐标原点

要为图层分配 WCS，请打开图层设置并使用 WCS 选择器。选择器旁边的快速编辑按钮让您无需离开对话框即可调整偏移。

没有明确分配 WCS 的图层使用工具栏下拉菜单中的全局活动 WCS。
