...
# Priority 1: Check if MSYS2_PATH is already set AND is a valid UCRT64 environment.
# This is the most reliable method and correctly handles properly configured CI runners.
if [ -n "$MSYS2_PATH" ] && [ -d "$MSYS2_PATH/ucrt64/bin" ]; then
    ...
    if [ -n "$WIN_MSYS2_PATH" ]; then
        ...
        if [ -d "$DETECTED_PATH/ucrt64/bin" ]; then
            ...
            echo "FATAL: Auto-detected path '$DETECTED_PATH' is not a valid MSYS2 UCRT64 installation."
            exit 1
        fi
    else
        echo "FATAL: Could not auto-detect MSYS2 installation path."
        echo "Please run this script from within an MSYS2 UCRT64 shell."
        exit 1
    fi
fi
...
if [[ "$1" == "pacman" || -z "$1" ]]; then
    ...
    echo "export PKG_CONFIG_PATH=$MSYS2_PATH/ucrt64/lib/pkgconfig" >> .msys2_env
    echo "export GI_TYPELIB_PATH=$MSYS2_PATH/ucrt64/lib/girepository-1.0" >> .msys2_env
    echo "export LD_LIBRARY_PATH=$MSYS2_PATH/ucrt64/lib" >> .msys2_env
    ...
    PACKAGES=(
      ...
      # These tools are in the base MSYS repository, not UCRT64
      autoconf
      automake
      libtool
      git
      ...
      mingw-w64-ucrt-x86_64-python-pyopengl-accelerate
      mingw-w64-ucrt-x86_64-python-scipy
      mingw-w64-ucrt-x86_64-python-svgelements
      mingw-w64-ucrt-x86_64-pyinstaller 

      # Pure Python dependencies needed for environment
      mingw-w64-ucrt-x86_64-python-aiohttp
      mingw-w64-ucrt-x86_64-python-blinker
      mingw-w64-ucrt-x86_64-python-platformdirs
      mingw-w64-ucrt-x86_64-python-pluggy
      mingw-w64-ucrt-x86_64-python-poetry-core
      mingw-w64-ucrt-x86_64-python-pytest-asyncio
      mingw-w64-ucrt-x86_64-python-pytest-cov
      mingw-w64-ucrt-x86_64-python-pytest-mock
      mingw-w64-ucrt-x86_64-python-websockets
...
if [[ "$1" == "pip" || -z "$1" ]]; then
    ...
    echo "export PYTHONPATH=$MSYS2_PATH/ucrt64/lib/$PYTHON_VERSION/site-packages:\$PYTHONPATH" >> .msys2_env
    source .msys2_env
