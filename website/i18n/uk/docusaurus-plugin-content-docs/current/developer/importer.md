...
Цей документ описує архітектуру системи імпорту файлів Rayforge, яка обробляє конвертацію різних
форматів файлів (SVG, DXF, PNG, PDF тощо.) в модель документа Rayforge.
...
Система імпорту побудована навколо чотирифазного конвеєра, який трансформує сирі файлові дані у
повністю позиціоновані об'єкти документа. Кожна фаза має специфічну відповідальність і виробляє
чітко визначені структури даних.
...
Конвертує пропарсені дані у векторні об'єкти `Geometry` відповідно до `VectorizationSpec`.
...
Розраховує матриці трансформації для відображення Нативних координат у Світові координати на основі
наміру користувача.
...
Інстанціює доменні об'єкти Rayforge (`WorkPiece`, `Layer`) на основі плану компонування.
...
Легковесне сканування, яке витягує метадані без повної обробки. Використовується для побудови UI для
імпортера, включаючи список вибору шарів. Це НЕ є частиною основного конвеєра імпорту, що
виконується `get_doc_items()`.
...
Конвеєр імпорту обробляє кілька систем координат через ретельну трансформацію:
...
Абстрактний базовий клас, що визначає інтерфейс для всіх імпортерів. Підкласи мають реалізувати
методи конвеєра та оголосити свої можливості через атрибут `features`.
