---
description:
  "在 Rayforge 中设置相机校准，以实现精确的工件对齐。使用您的相机预览和定位材料上的设计。"
---
...
设置相机既可以通过引导式[相机向导](#第-2-步相机向导)完成——一个涵盖图像设置、镜头校准和对齐的单一流程——也可以从相机属性面板手动配置每个区域。无论哪种方式，设置都涵盖四个区域：

1. **添加相机** — 连接相机并将其添加到机器配置中
2. **调整图像设置** — 调整亮度、对比度、白平衡和降噪
3. **校准镜头** — 使用相机向导的自动校准或手动系数校正畸变
4. **对齐相机** — 将相机像素映射到机器坐标以实现精确定位

相机属性面板一目了然地显示校准和对齐的状态图标：
...
**相机向导**以单一引导流程运行完整的相机设置，依次涵盖所有三个区域——图像设置、镜头校准和图像对齐。它从以下位置启动：

- 相机属性面板中的**相机向导**行 — 点击 **Start**
- 当您在其相机步骤启用相机并继续时，从[配置向导](../getting-started/first-time-setup.md)自动启动

### 第 2.1 步：调整图像设置

![图像设置对话框](/screenshots/machine-settings-camera-image-settings.webp)

图像设置是相机向导的第一阶段——向导在此打开，让您设置分辨率、白平衡、亮度、对比度和降噪。如果您没有运行向导，或想微调它设置的值，请点击相机属性中**图像设置**旁边的
**Configure** 以打开图像设置对话框。调整这些参数以获得清晰的相机视图：
...
YUYV 选项在您的相机使用默认 MJPEG 格式产生偏绿图像时很有用。请注意，YUYV 是未压缩的，可能会降低 USB
2.0 连接上可用的分辨率或帧率。

### 第 2.2 步：镜头校准

如果您的相机使用广角镜头或安装在倾斜角度，图像可能会显示可见的弯曲— 直线看起来弯曲，特别是在画面边缘附近。这称为镜头畸变，即使您的对齐点经过仔细测量，也可能影响对齐精度。

镜头校准是相机向导的第二阶段。它让您选择如何校正畸变：

- **Automatic** — 捕获打印校准卡的画面；向导为您计算畸变模型
- **Manual** — 手动输入径向（k1–k3）和切向（p1–p2）系数
- **Skip** — 不校正畸变；您可以稍后校准
...
3. 在相机视图内将卡片移动到不同的位置和角度，并为每个位置点击 **Capture
   Frame**。争取至少 8 次捕获，覆盖整个画面，包括角落和边缘。进度条和状态指示器显示捕获质量。
...
如需手动系数或在自动校准后微调结果，请点击相机属性中**镜头校准**旁边的 **Configure**
打开镜头校准对话框。在这里您可以手动调整畸变系数——微调径向（k1–k3）和切向（p1–p2）参数。
...
- **对号** — 对齐是最新且有效的
- **警告**
  — 需要重新对齐。当镜头校准更新时会发生这种情况，因为畸变校正会改变相机图像并使现有对齐无效。您的对齐点会保留 —只需打开对话框并再次点击**应用**即可。
...
Rayforge 支持本地摄像头，以及通过 HTTP 快照、HTTP/MJPEG 流和 RTSP 连接的网络摄像头。本地摄像头会自动检测；网络摄像头可直接输入 URL 添加。

### 添加网络摄像头

1. 打开 **机器 → 机器设置 → 摄像头**。
2. 点击 **+**，选择 **HTTP snapshot URL**、**HTTP stream URL** 或 **RTSP stream**。
3. 输入名称和 URL，例如：
   - HTTP 快照：`http://192.168.1.50:8080/media/getCapturePhoto`
   - HTTP/MJPEG 流：`http://192.168.1.50/mjpeg`
...
如果没有图像，请检查 IP 地址、URL 方案，以及摄像头和计算机是否位于可互相访问的同一网络。对于 HTTP 快照，Rayforge 会保留最后一张有效图像，并降低频率重试连接。
