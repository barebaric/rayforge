---
description:
  "Set up the Rayforge development environment. Build, run, and debug the application from source
  code."
---
...
See the [Installation Guide](../../getting-started/installation.mdx#linux-pixi) for Pixi
...
See the [Installation Guide](../../getting-started/installation.mdx#linux-pixi) for Pixi
installation instructions. Pixi supports macOS on Apple Silicon (`osx-arm64`).
...
See the [Installation Guide](../../getting-started/installation.mdx#windows-developer) for detailed
MSYS2 developer setup instructions.

### Quick Start

Development tasks on Windows are managed via the `run.bat` script, which is a wrapper for the MSYS2
shell.

After cloning the repository and completing the MSYS2 setup, you can use these commands from a
...
This executes `scripts/win/win_setup.sh` to install all necessary system and Python packages into
your MSYS2/UCRT64 environment.

### Pre-commit Hooks (Optional)

To automatically format and lint your code before each commit, run this from the MSYS2 UCRT64 shell:
...
Pre-commit hooks require running git commands from within the MSYS2 UCRT64 shell, not from
...
Alternatively, you can run the scripts directly from the MSYS2 UCRT64 shell:
